...
public class LongArraySequence implements Cloneable, Iterable<Long> {
    ...
    public void ensureCapacity( int minimumCapacity ) {
        ...
        // STUDENT IMPLEMENTATION GOES HERE
        long[] temp = new long[minimumCapacity];
    ...
    public LongArraySequence concatenation( LongArraySequence other ) {
        ...
        // STUDENT IMPLEMENTATION GOES HERE
        LongArraySequence newSequence = new LongArraySequence(other);

        if (this.used + other.used != 0) {
            newSequence.ensureCapacity(this.used + other.used);
        }
        //newSequence.ensureCapacity(this.used + other.used);
        for (int i = 0; i < this.used; i++) {
            newSequence.data[newSequence.used++] = this.data[i];
        }
        newSequence.cursor = -1;
        return newSequence;
    }
    ...
    public void addBefore( long newEntry ) {
        ...
        for (int i = this.used - 1; i >= this.cursor; i--) {
            this.data[i + 1] = this.data[i];
        }
        this.data[cursor] = newEntry;
        this.used++;
    }
    ...
    public void addAfter( long newEntry ) {
        if ( this.data.length == this.used )
            this.ensureCapacity( this.used * 2 );
        // STUDENT IMPLEMENTATION GOES HERE
        if (!this.isCurrent()) this.cursor = this.used;
        if ( this.cursor != this.used) cursor++;
        for (int i = this.used - 1; i >= this.cursor; i--) {
            this.data[i + 1] = this.data[i];
        }
        this.data[cursor] = newEntry;
    ...
    public void removeCurrent( ) {
        ...
        // STUDENT IMPLEMENTATION GOES HERE
        for (int i = this.cursor; i < this.used; i++) {
