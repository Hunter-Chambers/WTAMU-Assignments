...
//
    ...
    public boolean equals( Object other ) {
        ...
        /* cannot be equal if number of nodes differs or if hascode differes*/
        boolean usedMatch = candidate.used == this.used;
        boolean hashMatch = candidate.hashCode() == this.hashCode();

        if ( !(usedMatch && hashMatch) ) return false;

        DLNode<Item> thisList = this.head.getNext();
        DLNode<Item> otherList = candidate.head.getNext();
        boolean isEqual = true;
        //we now know that both lists have same number of nodes
        //STUDENT WORK COMES HERE to check for detailed equality
        //we must traverse both lists to check for a mismatch in data
        //and while so doing, check for cursor position mismatch
        while ((thisList != this.tail) && isEqual) {
            if (((thisList == this.cursor) && (otherList != candidate.cursor)) ||
                    ((thisList != this.cursor) && (otherList == candidate.cursor))) {
                isEqual = false;
            } else if (thisList.getData() != otherList.getData()) {
                isEqual = false;
            } else {
    ...
    public void addAfter( Item newValue ) {
        this.used++;
        //STUDENT WORK COMES HERE 
        if (!this.isCurrent()) this.end();
        DLNode<Item> newNode = new DLNode<Item>(this.cursor,newValue,this.cursor.getNext());
    ...
    public void addBefore( Item newValue ) {
        this.used++;
        //STUDENT WORK COMES HERE
        if (!this.isCurrent()) this.begin();
        DLNode<Item> newNode = new DLNode<Item>(this.cursor.getPrevious(),newValue,this.cursor);
    ...
    public DLinkedSequence<Item> concatenation( DLinkedSequence<Item> other ) {
        ...
        while (thisPtr != this.tail) {
            newSequence.addAfter(thisPtr.getData());
            thisPtr = thisPtr.getNext();
        }

        while (otherPtr != other.tail) {
            newSequence.addAfter(otherPtr.getData());
