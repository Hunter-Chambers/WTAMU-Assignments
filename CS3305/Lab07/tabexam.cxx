...
//
// This version of the exam program includes an improved test for heap
...
// everything is included in one file for easier distribution.
...
#include "table.h"    // Provides the table template class.
...
bool correct(const table<TRec>& test, double right_data[] ) {
    ...
    for (i = 0; answer && (i <= MAX_KEY); ++i) {
        ...
        //if ( should_be_present ) cout << "Key " << i << " should be present\n";
        is_present_result = test.is_present(i);
        //if ( is_present_result ) cout << "Key " << i << " is present\n";
        test.find(i, find_result, find_record);
        //if ( find_result ) cout << "Key " << i << " was found\n";
        // Check that the boolean values are correct
        if (should_be_present && !is_present_result)
            answer = false;
        else if (should_be_present && !find_result)
            answer = false;
        else if (should_be_present && (right_data[i] != find_record.data))
            answer = false;
        // Update the count of the size of the table
        if (should_be_present && is_present_result && find_result) n++;
    }
    cout << "Table size from detailed analysis is " << n << "\n";
    cout << "Table size from size() is " << test.size() << "\n";
    if (test.size( ) != n)
        answer = false;
...
int test1( ) {
    ...
    const size_t TESTSIZE = 3000; //3000; // Size of a large test.
    ...
    cout << test;
    cout << endl;
    if (correct(test, right_data)) points_earned += 4;
    cout << "You have now earned " << points_earned << " points for this test\n";
    ...
    cout << endl;
    test_record.key = 4;
    test_record.data = right_data[4] = 40.0;
    ...
    cout << endl;
    test_record.key = 813;
    test_record.data = right_data[813] = 8130.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 4;
    ...
    cout << endl;
    test_record.key = 4;
    test_record.data = right_data[4] = 400.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 4;
    ...
    cout << endl;
    test_record.key = 811;
    test_record.data = right_data[811] = 8110.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 4;
    ...
    cout << endl;
    test_record.key = 810;
    test_record.data = right_data[810] = 8100.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 4;
    ...
    test_record.data = right_data[1611] = 16110.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 4;
    ...
    cout << endl;
    test_record.key = 0;
    test_record.data = right_data[0] = 42.0;
    test.insert(test_record);
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 7;
    cout << "You have now earned " << points_earned << " points for this test\n";
    ...
    for (i = 6; i <= TESTSIZE; i++) {
        ...
        test_record.data = right_data[next] = next/10.0;
        test.insert(test_record);
    }
    cout << "The table is:\n";
    cout << test;
    if (correct(test, right_data)) points_earned += 9;
    cout << "You have now earned " << points_earned << " points for this test\n";
...
// returns 20. Otherwise the function returns zero.
    ...
    const size_t TESTSIZE = 500; // Size of a large test.
    ...
    test.insert(test_record);
    test_record.key = 5;
    test_record.data = 50.0;
    test.insert(test_record);
    test_record.key = 6;
    test_record.data = right_data[6] = 60.0;
    test.insert(test_record);
    test.remove(5);
    if (!correct(test, right_data)) return 0;
    ...
    //empty table of previous entries
    test.remove(4); test.remove(6); right_data[4] = right_data[6] = -1;
    //seed random number generator with system clock
    std::srand( time(NULL) );
    for (i = 0; i < TESTSIZE; i++) {
        //cycle until we get a unique key
        do {
            next = rand( ) % TABLE_SIZE; //(MAX_KEY + 1);
        } while ( test.is_present(next) );
        test_record.key = next;
        test_record.data = right_data[next] = next/10.0;
        test.insert(test_record);
    }
    //cout << "Table after loading it\n" << test;
    for (i = 0; i < MAX_KEY; i++) {
        if (rand( ) % 2 == 0) {
            test.remove(i);
            right_data[i] = -1;
    ...
    //cout << "Table after removing about half the entries it\n" << test;
    if (!correct(test, right_data)) return 0;
...
int test4( ) {
    ...
    for (i = 4; i <= 7; i++) {
        test_record.key = i;
        test_record.data = right_data[i] = i/10.0;
        test.insert(test_record);
    }
...
int main( ) {
    ...
    cout << "If you submit this table to Professor Haiduk now, you will have\n";
    cout << "at most " << sum << "  points out of the " << POINTS[0];
